...
<!-- JavaScript code below should be in a separate file named unified-auth.js -->

// Lưu ý: Dữ liệu người dùng (đăng ký và trạng thái đăng nhập) đang được lưu trữ trong Local Storage của trình duyệt.
// Mật khẩu được mã hóa (hashed) bằng SHA256 trước khi lưu, nhưng đây chỉ là biện pháp bảo vệ cơ bản trên client-side.
// Local Storage là bộ nhớ cục bộ chỉ tồn tại trên thiết bị/trình duyệt hiện tại và có thể bị truy cập bởi các script khác trên cùng origin.
// Do đó, tài khoản đăng ký trên máy tính sẽ không thể đăng nhập trên điện thoại (hoặc trình duyệt khác) vì dữ liệu không được đồng bộ.
// Để hỗ trợ đăng nhập đa thiết bị và bảo mật mạnh mẽ hơn, cần sử dụng cơ sở dữ liệu phía máy chủ (backend) với phương pháp hashing mật khẩu an toàn hơn (ví dụ: bcrypt).
...
// Lưu ý: Chỉ lưu username, không lưu mật khẩu (dù đã hash) vào thông tin người dùng hiện tại.
...
function login() {
  ...
  // Mã hóa mật khẩu nhập vào để so sánh với mật khẩu đã lưu (đã mã hóa)
  const hashedPassword = CryptoJS.SHA256(password).toString();

  // Tìm người dùng khớp cả tên đăng nhập và mật khẩu đã mã hóa
  const foundUser = registeredUsers.find(user =>
    user.username === username && user.password === hashedPassword
  );

  if (foundUser) {
    // Lưu thông tin người dùng đang đăng nhập (chỉ username)
...
function register() {
  ...
  if (userExists) {
    ...
    // Mã hóa mật khẩu trước khi lưu vào Local Storage
    const hashedPassword = CryptoJS.SHA256(password).toString();
    
    // Tạo người dùng mới và lưu vào Local Storage
    // Lưu ý: Dữ liệu này chỉ tồn tại trên trình duyệt/thiết bị hiện tại.
    const newUser = { username, password: hashedPassword }; // Lưu username và mật khẩu đã mã hóa
...
$(document).ready(function() {
  ...
  // Nếu chưa đăng nhập, không làm gì cả, form đăng nhập sẽ hiển thị mặc định
});