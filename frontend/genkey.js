...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Kiểm tra số dư
  const BIN_ID = "683709868561e97a501c9633";
  ...
  .then(res => res.json())
  .then(data => {
    const balance = data.record.balance;
    if (balance < 50000) {
  ...
  // Thêm các hàm toggleHistory, clearHistory, logout nếu có trong genkey.js hoặc unified-auth.js
});
