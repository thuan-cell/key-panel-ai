...
const STATUS_FILE = IS_RENDER
  ? path.join("/tmp", "admin_status.json")
  : path.join(__dirname, "admin_status.json");

// API kiểm tra quyền admin từ xa
app.get("/api/access-status", async (req, res) => {
  try {
    const raw = await fs.readFile(STATUS_FILE, "utf-8");
    const json = JSON.parse(raw);
    res.json({ admin_granted: !!json.admin_granted });
  } catch {
    res.json({ admin_granted: false });
  }
});

// API bật quyền admin
app.post("/api/grant-access", async (req, res) => {
  await fs.writeFile(STATUS_FILE, JSON.stringify({ admin_granted: true }), "utf-8");
  res.send("✅ Quyền truy cập đã được cấp");
});

// API thu hồi quyền admin
app.post("/api/revoke-access", async (req, res) => {
  await fs.writeFile(STATUS_FILE, JSON.stringify({ admin_granted: false }), "utf-8");
  res.send("🚫 Quyền truy cập đã bị thu hồi");
});
