...
// Danh sách user (không khuyến nghị công khai)
app.get("/api/users", async (req, res) => {
  try {
    const users = await readUsers();
    if (users.length === 0) return res.status(404).json({ message: "Không có người dùng nào" });
    res.json(users);
  } catch (err) {
...
app.post("/api/grant-access", async (req, res) => {
  try {
    ...
    res.json({ message: "Đã cấp quyền truy cập" });
  } catch (err) {
    res.status(500).json({ message: "Lỗi khi ghi quyền truy cập" });
  }
});
...
app.get("/api/access-status", async (req, res) => {
  ...
  res.json({ admin_granted: granted });
});
